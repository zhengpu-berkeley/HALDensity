...
def _compute_density_l2_from_grid(density_grid: np.ndarray, delta_j: np.ndarray) -> float:
    return float(np.sum(np.square(density_grid) * delta_j))


def _state_to_target_grid(state: DensityL2TMLEState) -> RCTargetGrid:
    return RCTargetGrid(
        t0=float("nan"),
...
def _build_unbounded_wald_interval(center: float, standard_error: float) -> tuple[float, float]:
    return float(center - 1.96 * standard_error), float(center + 1.96 * standard_error)


def _evaluate_tail_density_l2(
    target_grid: RCTargetGrid,
    points: np.ndarray,
    survival_clip: float,
) -> np.ndarray:
    tail_l2 = np.cumsum((np.square(target_grid.density_grid) * target_grid.delta_j)[::-1])[
        ::-1
    ]
    edge_tail_l2 = np.concatenate((tail_l2, [0.0]))
...
def _evaluate_targeting_gbar(
    ...
    *,
    ...
    vals = np.asarray(censoring_cache.km.predict(t), dtype=float)
    return np.maximum(vals, float(max(targeting_gbar_floor, censoring_cache.clip)))


def _compute_density_l2_direction_on_grid(
    target_grid: RCTargetGrid,
    censoring_cache: RCCensoringCache,
    survival_clip: float,
    ...
    if jump_times.size == 0:
        ...
        tail_density_l2_jump = _evaluate_tail_density_l2(
            target_grid,
            jump_times,
            survival_clip,
        )
...
def _initialize_density_l2_state(
    initial_fit: RCInitialFit,
    censoring_cache: RCCensoringCache,
) -> DensityL2TMLEState:
    target_grid = _initial_fit_to_target_grid(initial_fit)
    psi_init = _compute_density_l2_from_grid(target_grid.density_grid, target_grid.delta_j)
    return DensityL2TMLEState(
...
def _evaluate_density_l2_state(
    state: DensityL2TMLEState,
    observed_data: pd.DataFrame,
    survival_clip: float,
    targeting_gbar_floor: float,
) -> tuple[DensityL2TMLEState, dict[str, Any]]:
    target_grid = _state_to_target_grid(state)
    raw_direction, centered_direction, direction_details = _compute_density_l2_direction_on_grid(
        target_grid,
    ...
    psi_current = _compute_density_l2_from_grid(state.density_grid, state.delta_j)
    evaluated_state = DensityL2TMLEState(
        grid_edges=state.grid_edges.copy(),
        grid_midpoints=state.grid_midpoints.copy(),
        delta_j=state.delta_j.copy(),
        log_density_grid=state.log_density_grid.copy(),
...
def _one_density_l2_tmle_update(
    current_state: DensityL2TMLEState,
    observed_data: pd.DataFrame,
    *,
    ...
    if current_state.centered_direction is None:
        raise ValueError(
            "current_state must be evaluated before calling _one_density_l2_tmle_update."
        )
    ...
    psi_star = _compute_density_l2_from_grid(density_star, current_state.delta_j)
    updated_state = DensityL2TMLEState(
...
def _iterate_density_l2_tmle(
    ...
    *,
    ...
    state = _initialize_density_l2_state(initial_fit, censoring_cache)
    ...
    for iteration in range(1, max_iter + 1):
        state, _ = _evaluate_density_l2_state(
            state,
            observed_data,
            survival_clip,
            targeting_gbar_floor,
        )
        updated_state, update_info = _one_density_l2_tmle_update(
            state,
            observed_data,
            eps_bracket_start=eps_bracket_start,
            eps_bracket_growth=eps_bracket_growth,
    ...
    if final_state.eif_values is None:
        raise RuntimeError("Iterative density-L2 TMLE final state is missing EIC values.")
...
def _summarize_density_l2_tmle_step(
...
class RightCensoredDensityL2TargetLearner:
    """HAL-TMLE targeter for Psi(P) = integral p(t)^2 dt under right censoring."""
    ...
    def run_m_step(
        ...
        *,
        ...
        init_state = _initialize_density_l2_state(initial_fit, censoring_cache)
        init_state, _ = _evaluate_density_l2_state(
            init_state,
            observed_data,
            self.survival_clip,
            targeting_gbar_floor,
        ...
        if init_state.eif_values is None:
            raise RuntimeError("Initial density-L2 TMLE state is missing EIC values.")
        ...
        init_filtered_var = float(init_filtered_eic["estimand_variance"])
        init_filtered_se = float(init_filtered_eic["standard_error"])
        ...
        if mode == "auto" and passes_initial_threshold:
            ...
            final_filtered_var = float(init_filtered_var)
            final_filtered_se = float(init_filtered_se)
            ...
            one_step_stage = {
                ...
                "estimand_variance": float(init_var_all_eic),
                "standard_error": float(init_se_all_eic),
                "estimand_variance_all_eic": float(init_var_all_eic),
                "standard_error_all_eic": float(init_se_all_eic),
                "estimand_variance_clip_active_filtered": float(init_filtered_var),
                "standard_error_clip_active_filtered": float(init_filtered_se),
            ...
            one_step_bundle = _iterate_density_l2_tmle(
            ...
            one_step_stage = _summarize_density_l2_tmle_step(
                one_step_bundle,
                step=1,
                min_score_tol=self.min_score_tol,
            )
            one_step_state: DensityL2TMLEState = one_step_bundle["final_state"]
            if one_step_state.eif_values is None:
                raise RuntimeError("One-step density-L2 TMLE state is missing EIC values.")
            ...
            one_step_stage["estimand_variance_clip_active_filtered"] = float(
                one_step_filtered_eic["estimand_variance"]
            )
            one_step_stage["standard_error_clip_active_filtered"] = float(
                one_step_filtered_eic["standard_error"]
            )
            ...
            if mode == "one_step" or not run_full_iterative:
                ...
                final_bundle = _iterate_density_l2_tmle(
                    initial_fit=initial_fit,
                    censoring_cache=censoring_cache,
                    observed_data=observed_data,
                    survival_clip=self.survival_clip,
            ...
            if final_state.eif_values is None:
                raise RuntimeError("Final density-L2 TMLE state is missing EIC values.")
            ...
            final_filtered_var = float(final_filtered_eic["estimand_variance"])
            final_filtered_se = float(final_filtered_eic["standard_error"])
        ...
        ci_lower, ci_upper = _build_unbounded_wald_interval_allow_nan(
            final_psi,
            init_se_all_eic,
        )
            ...
            [
                {
                    ...
                    "psi_star": final_psi,
                    "density_l2_init": float(init_state.psi_current),
                    "density_l2_star": final_psi,
                    "density_l2_one_step": float(one_step_stage["psi_star"]),
                    "density_l2_final": final_psi,
                    ...
                    "estimand_variance_initial_stage": float(init_var_all_eic),
                    "standard_error_initial_stage": float(init_se_all_eic),
                    "estimand_variance_all_eic_initial_stage": float(init_var_all_eic),
                    "standard_error_all_eic_initial_stage": float(init_se_all_eic),
                    "estimand_variance_clip_active_filtered_initial_stage": float(
                        init_filtered_var
                    ),
                    "standard_error_clip_active_filtered_initial_stage": float(
                        init_filtered_se
                    ),
                    ...
                    "estimand_variance_clip_active_filtered_one_step": float(
                        one_step_stage["estimand_variance_clip_active_filtered"]
                    ),
                    "standard_error_clip_active_filtered_one_step": float(
                        one_step_stage["standard_error_clip_active_filtered"]
                    ),
                    ...
                    "estimand_variance_final": final_var_all_eic,
                    "standard_error_final": final_se_all_eic,
                    "estimand_variance_all_eic_final": final_var_all_eic,
                    "standard_error_all_eic_final": final_se_all_eic,
                    "estimand_variance_clip_active_filtered_final": float(
                        final_filtered_var
                    ),
                    "standard_error_clip_active_filtered_final": float(
                        final_filtered_se
                    ),
        ...
        fit_row: dict[str, Any] = {
            ...
            "one_step_stage": {
                ...
                "estimand_variance_clip_active_filtered": float(
                    one_step_stage["estimand_variance_clip_active_filtered"]
                ),
                "standard_error_clip_active_filtered": float(
                    one_step_stage["standard_error_clip_active_filtered"]
                ),
                "targeting_gbar_floor": float(targeting_gbar_floor),
            ...
            "final_stage": {
                ...
                "epsilon_last": final_epsilon,
                "estimand_variance": final_var_all_eic,
                "standard_error": final_se_all_eic,
                "estimand_variance_all_eic": final_var_all_eic,
                "standard_error_all_eic": final_se_all_eic,
                "estimand_variance_clip_active_filtered": float(final_filtered_var),
                "standard_error_clip_active_filtered": float(final_filtered_se),
                "targeting_gbar_floor": float(targeting_gbar_floor),
...
def right_censored_density_l2_targeting_M_step(
    initial_estimator: RightCensoredInitEstimator,
    observed_data: pd.DataFrame,
    *,
    km: Optional[KaplanMeier] = None,
...
def right_censored_density_l2_estimand_variance(
    targeted_fit: dict[str, Any],
    observed_data: Optional[pd.DataFrame] = None,
    which: str = "default",
    **kwargs: Any,
) -> np.ndarray:
    learner = RightCensoredDensityL2TargetLearner(**kwargs)
